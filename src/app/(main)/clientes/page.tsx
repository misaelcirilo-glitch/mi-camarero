      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <div className="flex items-start justify-between">
              <div><p className="text-xs font-bold text-slate-400 uppercase">{s.label}</p><p className={`text-2xl font-black mt-1 ${s.color}`}>{s.value}</p></div>
                ...
                {customers.length === 0 ? (
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3 text-center">
                      <button onClick={() => { setEditingCustomer(c); setEditPoints(c.points) }} className="inline-flex items-center gap-1 bg-purple-50 text-purple-700 px-2 py-1 rounded-lg text-xs font-bold hover:bg-purple-100">
          ...
          {showRewardForm && (
            <form onSubmit={handleAddReward} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Valor</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {rewards.map(r => (
                <div key={r.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                  <div className="flex items-start justify-between">
                    <div className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center"><Gift size={20} className="text-purple-600" /></div>
                    <button onClick={() => handleDeleteReward(r.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
                  </div>
                  <h3 className="font-bold text-slate-800 mt-3">{r.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="bg-purple-100 text-purple-700 text-xs font-bold px-2 py-0.5 rounded-lg">{r.points_required} pts</span>
