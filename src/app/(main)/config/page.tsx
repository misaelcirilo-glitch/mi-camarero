...
export default function ConfigPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2"><Settings className="text-slate-500" size={24} /> Configuracion</h1>
        <p className="text-sm text-slate-500 mt-1">Ajustes de tu restaurante</p>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-bold text-slate-800">Plan {plan?.displayName || 'Starter'}</span>
            ...
            {tenant?.subscriptionStatus === 'trial' && tenant?.trialEndsAt && (
              <p className="text-xs text-orange-600 font-bold mt-1">
                Trial termina el {new Date(tenant.trialEndsAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}
              </p>
            )}
          </div>
          <Link href="/planes" className="flex items-center gap-1.5 bg-white text-orange-600 px-4 py-2 rounded-xl text-xs font-bold border border-orange-200 hover:bg-orange-50 transition-colors">
            <ExternalLink size={14} /> Cambiar plan
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
        <h2 className="font-bold text-slate-800">Datos del restaurante</h2>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">URL publica</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Estado suscripcion</label>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <h2 className="font-bold text-slate-800 mb-4">Funcionalidades de tu plan</h2>
        ...
        {!plan?.features?.onlineOrdering && (
          <Link href="/planes" className="inline-block mt-4 text-xs font-bold text-orange-500 hover:underline">Mejora tu plan para desbloquear mas funcionalidades</Link>
        )}
      </div>
    </div>
  )
